...
if(isset($_SESSION['carrito_ventas'])):
	if(isset($_POST['IdProd']) && $_POST['action']=="Eliminar"):
		$arreglo=$_SESSION['carrito_ventas'];
		$enc=false;
		$num=0;
		...
		if($enc==true):
			...
			$_SESSION['carrito_ventas']=$arreglo;
		endif;
	endif;
	...
	if(isset($_POST['IdP']) && empty($_SESSION['carrito_ventas'])):
		$SubTotal = $_POST['precio'] * $_POST['cant'];
		$Igv=(($SubTotal / 1.18) * 0.18);
...
<table class='table table-striped table-bordered table-condensed table-hover'>
	...
	<?php if(!empty($_SESSION['carrito_ventas'])): ?>
		<?php $suma=0; $cant=0; ?>
		<?php foreach ($_SESSION['carrito_ventas'] as $key):?>
		<?php $suma= $suma+$key['SubTotal']; 
			$cant=$cant+1;?>
		<tr>
			<td class="text-center"><?=$cant?></td>
			<td><?=$key['Producto']?></td>
			<td class="text-center">S/. <?=$key['Precio']?></td>
			...
			<td class="text-center">
				<span title="Incrementar Item" class="btn btn-xs btn-success" onclick="AgregarVentas('<?=$key['Id']?>','Agregar')">
					<i class="glyphicon glyphicon-plus"></i>
				</span>
				<span title="Disminuir Item" class="btn btn-xs btn-warning" onclick="QuitarVentas('<?=$key['Id']?>','Quitar')">
					<i class="glyphicon glyphicon-minus"></i>
				</span>
				<span title="Borrar Item" class="btn btn-xs btn-danger" onclick="EliminarVentas('<?=$key['Id']?>','Eliminar')">
...
<table class='table table-striped table-bordered table-condensed table-hover'>
	...
	<tbody>
		<tr>
			...
			<td class="text-center">S/. <?=number_format($suma,2)?></td>
		</tr>
	</tbody>
</table>